...
export default function Report_list() {
    ...
    const[controls,setcontrolles]=useState('hidden');
    useEffect(() => {
        getData();
    }, []);
    ...
    return (
        <div>
            ...
            <div className="bg-white h-auto mt-20 rounded-[25px] bg-opacity-25 mx-[5%]">
                {loading ? (
                    <p className="text-center text-white py-10  ">Loading reports...</p>
                ) : reports.length > 0 ? ( 
                    <div className="flex flex-col items-center">
                        {reports.map((report, index) => (
                            <div
                                key={index}
                                className="bg-white mt-5 mb-5 p-5 h-auto w-full max-w-[90%] rounded-[50px] flex flex-wrap items-center justify-between gap-4 shadow-lg"
                            >
                                {/* ID Section */}
                                <div className="flex-1 text-center hidden sm:block sm:w-[20%]">
                                    <h1 className="text-black text-xs sm:text-base font-bold">
                                        ID {report.report_id}
                                    </h1>
                                </div>

                                {/* Email Section */}
                                <div className="flex-1 text-center hidden md:block sm:w-[20%]">
                                    <h1 className="text-black text-xs sm:text-base font-bold">
                                        {report.u_id}
                                    </h1>
                                </div>
                                ...
                                <div className="w-full sm:w-auto flex justify-center sm:justify-end">
                                    <h1
                                        className="bg-blue-500 flex items-center justify-center px-4 py-2 text-xs sm:text-base font-bold text-white rounded-3xl cursor-pointer hover:bg-green-600 transition-all"
